...
void letterEffect(String a, String b)
{  
  String dyn = a;
  String lu = b;
...
void laserShow(String a, String b)
{
  String str = a;
  int w = Drawing::stringAdvance(str);
  int w2 = Drawing::stringAdvance(b);
  int count = 100;
  int angle = 0;
  laser.setScale(0.5);
  for (int i = 0;i<count;i++) {
    ...
    laser.setMatrix(world);
    laser.setOffset(1024,1024 - 600);
    Drawing::drawString(b,-w2/2,-500, 1);
    angle += 8;
  }
  laser.setEnable3D(false);
}
...
{
  long centerX, centerY, w,h;
  Drawing::calcObjectBox(amiga_logo, sizeof(amiga_logo)/4, centerX, centerY, w, h);

  int count = 200;
  laser.setScale(4096/(float)w);
  laser.setOffset(2048,2048);
  ...
  for (int i = 0;i<count;i++) {
    ...
    maxMove += 400;
    Drawing::drawObject(amiga_logo, sizeof(amiga_logo)/4, -centerX, -centerY);
...
void drawC64Rotate()
{
  ...
  Drawing::calcObjectBox(draw_c64, sizeof(draw_c64)/4, centerX, centerY, w, h);
  int angle = 0;
  int x = -centerX;
  for (;x<4696;) {
...
// arduino + heart
void drawWeLove(const char* str)
{
  ...
  for (int i = 0;i<300;i++) {
    ...
    maxMove += 200;
    laser.setScale(0.4);
    Drawing::drawString(str,-w1/2, SIN((i*10) % 360)/100.);
    if (i>100) {
      laser.resetMaxMove();
...
void drawObjects()
{
  ...
  for (int i = 0;i<count;i++) Drawing::drawObject(amiga_logo, sizeof(amiga_logo)/4);
...
void drawC64()
{
  ...
  int angle = 0;
  for (int i = 0;i<count;i++) {
    Drawing::drawObjectRotated3D(draw_c64, sizeof(draw_c64)/4, centerX, centerY, angle % 360, 0, 5000);
    angle += 4;
  }
}
...
void loop() {
  ...
  presents("PRESENTS");
  drawPlane();
  laserShow("RADWAR", "PARTY");
  presents("DEMO");
  drawWeLove("2025");
  drawScroller(String("CHEERS TO EVERYONE!"),0.5,2048,100);
  drawAmigaV();
  drawLogo();
  presents("FOREVER");
  drawWeLove("TCC DESIGN");
  laserShow("RED", "SECTOR");
  drawC64Rotate();
  whatAbout3D();
  rotateCube(400);
  drawBike();
